...
public static class Day19
{
    ...
    private static ulong DesignCombinations(string pattern, ConcurrentDictionary<string, ulong> cache, HashSet<string> towels)
        => cache.TryGetValue(pattern, out var value) 
            ? value 
            : cache.GetOrAdd(pattern, towels.Where(pattern.StartsWith).BigSum(t => DesignCombinations(pattern[t.Length..], cache, towels)));
